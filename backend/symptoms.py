...
# Corrected symptoms list (MUST be 132)
symptoms_dict = {
...
13: 'Hemorrhoids(piles)', 18: 'Heart attack', 39: 'Varicose veins', 
...
# Load files
description_df = pd.read_csv(os.path.join(script_dir, 'HealthPredict', 'description.csv'))
precautions_df = pd.read_csv(os.path.join(script_dir, 'HealthPredict', 'precautions_df.csv'))
medications_df = pd.read_csv(os.path.join(script_dir, 'HealthPredict', 'medications.csv'))
diets_df = pd.read_csv(os.path.join(script_dir, 'HealthPredict', 'diets.csv'))
workout_df = pd.read_csv(os.path.join(script_dir, 'HealthPredict', 'workout_df.csv'))

model = pickle.load(open(os.path.join(script_dir, "aimodels", "svc.pkl"), "rb"))
...
def get_predicted_value(symptoms):
    ...
    print(f"Predicted disease index: {pred}, label: {disease}", file=sys.stderr)
    return disease

data = sys.argv[3]
payload = json.loads(data)
symptoms = payload['data'].split(",") if isinstance(payload['data'], str) else payload['data']
symptoms = [s.strip() for s in symptoms]

pred_disease = get_predicted_value(symptoms)

desc = " ".join(description_df[description_df["Disease"] == pred_disease]["Description"].values)

# Clean precautions (remove NaN)
pre_data = precautions_df[precautions_df['Disease'] == pred_disease][['Precaution_1','Precaution_2','Precaution_3','Precaution_4']].values.tolist()
my_precautions = [p for p in pre_data[0] if isinstance(p, str)]

# Convert medication list properly
med_data = medications_df[medications_df["Disease"] == pred_disease]["Medication"].values.tolist()
medications = eval(med_data[0]) if "[" in med_data[0] else med_data

# Convert diet list properly
diet_data = diets_df[diets_df["Disease"] == pred_disease]["Diet"].values.tolist()
rec_diet = eval(diet_data[0]) if "[" in diet_data[0] else diet_data

workout = workout_df[workout_df["disease"] == pred_disease]["workout"].values.tolist()
